...
int[] cashTillBackUp = new int[] { 0, 0, 0, 0 };
int registerCheckTillTotal = 0;

// registerDailyStartingCash: $1 x 50, $5 x 20, $10 x 10, $20 x 5 => ($350 total)
int[,] registerDailyStartingCash = new int[,] { { 1, 50 }, { 5, 20 }, { 10, 10 }, { 20, 5 } };

int[] testData = new int[] { 6, 10, 48, 20, 31, 36, 40, 41 };
int testCounter = 0;
...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    int updatedTillFour = 0;
    int updatedTillThree = 0;
    int updatedTillTwo = 0;
    int updatedTillOne = 0;

    // Accept the cash if the amount paid is correct.
    if (changeNeeded < 0)
    {
    ...
    {
        cashTill[3] += twenties;
        updatedTillFour += twenties;

        cashTill[2] += tens;
        updatedTillThree += tens;

        cashTill[1] += fives;
        updatedTillTwo += fives;
    ...
    if (changeNeeded > 0)
    {
        // Rollback the changes made to the till(s) if we run out of cash.

        cashTill[3] -= updatedTillFour;
        cashTill[2] -= updatedTillThree;
        cashTill[1] -= updatedTillTwo;
        cashTill[0] -= updatedTillOne;
